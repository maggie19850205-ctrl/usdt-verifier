...
function generateFallbackContent(name, typeId, rng) {
  ...
  if (typeId === 'complete-guide') {
    ...
    const items = [];
    for (let i = 0; i < 8; i++) {
      items.push(`�?${verbs[Math.floor(rng() * verbs.length)]} your ${name.toLowerCase()} strategy - ${benefits[Math.floor(rng() * benefits.length)]}`);
      rng();
    }
    return items.join('\n');
...
<a href="javascript:void(0)" onclick="openModal('${fullName.replace(/'/g, "\\'")}', ${type.price}, '${baseSlug + '-' + type.id}')" class="cta-btn">Buy Now - $${type.price.toFixed(2)}</a>
</div>
