...
const PRODUCT_FILES = {
  'usdt-payment-gateway-self-hosted': 'usdt-payment-gateway-self-hosted.zip',
  'ai-workforce-pro': 'ai-workforce-pro.zip',
  'automation-empire': 'automation-empire.zip',
  'geo-dominance-toolkit': 'geo-dominance-toolkit.zip',
  'infinite-knowledge-factory': 'infinite-knowledge-factory.zip',
  'ai-company-builder': 'ai-company-builder.zip',
  'context-vault': 'context-vault.zip',
  'anti-block-data-harvester': 'anti-block-data-harvester.zip',
  'market-intelligence-engine': 'market-intelligence-engine.zip',
      ...
      if (path === '/api/verify-payment' && method === 'POST') {
        ...
        if (result.valid && result.amountMatch) {
          ...
          const order = {
            ...
            directUrl: match ? `${SITE_URL}/downloads/${match}` : null, timestamp: new Date().toISOString(), fromAddress: result.from,
          };
          let orders = [];
